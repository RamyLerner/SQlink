#include <linux/init.h>    /* for __int, __exit*/
...
static int write_message(unsigned long user_msg, struct mq_t* mq){
	...
	if (ret != 0) {
		pr_err("Error on copy_from_user(struct message), return is %d\n", ret);
		goto err_return;
	}
	pr_info("local struct message created\n");	
	
	//allocate message buffer
	msgBuff = (char*)kmalloc(local_msg.size, GFP_KERNEL);
	if (IS_ERR(msgBuff)) {
		pr_err("Error on message buffer allocation\n");
		ret = PTR_ERR(msgBuff);
		goto err_return;
	}
	pr_info("Message buffer allocated in address: %p\n", msgBuff);

	//copy message data to message buffer
	ret = copy_from_user(msgBuff, local_msg.data, local_msg.size);
	if (ret !=0) {
		pr_err("Error on copying message data from user\n");
		goto err_dealloc_buff;
	}
	pr_info("Message buffer coppied from user\n");
	...
	mq -> numOfMessages++;
	pr_info("Message added succefylly\n");
...
static int read_message(unsigned long user_msg, struct mq_t* mq){
	...
	ret = wait_event_interruptible (mq -> sleeping_queue, list_empty (&mq -> head) == 0);	
	pr_info("%s: starting read\n", THIS_MODULE->name);
	first_msg = list_entry((&mq -> head) -> prev, struct message_link_t, node);
	pr_info("%s: got first_msg\n", THIS_MODULE->name);
	
	pr_info("willing to copy_to_user from address: %p\n", first_msg->data);
